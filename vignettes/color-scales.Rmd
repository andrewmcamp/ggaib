...
`scale_color_aib_b()` and `scale_fill_aib_b()` discretize a continuous
variable into bins and display a clean, discrete legend. This is
generally preferred over a smooth gradient because binned colors are
easier to read and compare across data points.
...
`scale_color_aib_c()` and `scale_fill_aib_c()` create a smooth
two-color gradient. These are available when a continuous colorbar is
needed, but in most cases the binned scales above are a better choice
for readability.
...
| `scale_color_aib_b()` / `scale_colour_aib_b()` | color | Binned |
| `scale_fill_aib_b()` | fill | Binned |
